...
const {
  ...
  refreshTokenService,
  refreshSuperAdminTokenService,
  logInSuperAdminService,
  changepasswordSuperAdminService,
  setpasswordService,
} = userService;
...
export const changePassword = async (req, res, next) => {
  try {
    ...
    const change = await changepasswordService(
      req,
      oldPassword,
      newPassword,
      conformPassword,
    ...
    return res
      .status(500)
      .json(sendMessage(false, "Interanal server error", null));
  }
};
...
export const changePasswordSuperAdmin = async (req, res, next) => {
  try {
    const { oldPassword, newPassword, conformPassword, email } = req.body;
    const change = await changepasswordSuperAdminService(
      req,
      oldPassword,
      newPassword,
      conformPassword,
    ...
    return res
      .status(500)
      .json(sendMessage(false, "Interanal server error", error));
  }
};

...
class Service {
  ...
  refreshSuperAdminTokenService = async (token, email) => {
    ...
    const refreshToken = verify(token, adminRefreshTokenKey, (err, decode) => {
      ...
      if (decode) {
        ...
        const accessToken = jwt.sign(payload, adminAccessTokenKey, {
          expiresIn: adminAccessTokenExpireTime,
        });
        return sendMessage(true, "access token refreshed", {
          token: accessToken,
          ...payload,
        });
      }
    ...
    try {
      await passport.authenticate(
        ...
        async (err, user, info) => {
          ...
          const refreshToken = jwt.sign(payload, adminRefreshTokenKey, {
            expiresIn: adminRefreshTokenExpireTime,
          });
  ...
  changepasswordSuperAdminService = async (
    req,
    oldPassword,
    newPassword,
    conformPassword,
    ...
    if (!oldPassword || !newPassword || !conformPassword || !email) {
      return sendMessage(false, "Please fill all the fields", {
        oldPassword,
        newPassword,
        conformPassword,
        email,
    ...
    if (newPassword !== conformPassword)
      return sendMessage(false, "Please correct the confirm password", {
        oldPassword,
        newPassword,
        conformPassword,
        email,
    ...
    if (newPassword.length < 6)
      return sendMessage(false, "Password legth must be 6 characters", {
        oldPassword,
        newPassword,
        conformPassword,
        email,
    ...
    if (req.user) {
      ...
      if (!user) {
        return sendMessage(false, "User not found", {
          oldPassword,
          newPassword,
          conformPassword,
          email,
  ...
  verifyEmailService = async (email) => {
    ...
    if (Otp) {
      ...
      return sendMessage(true, "OTP resended", {
    ...
    const savedOTP = await otp.save();
    if (savedOTP) {
      return sendMessage(true, "OTP sent successfully", {
        email: savedOTP.email,
        id: savedOTP._id,
        replacements,
      });
  ...
  signUpService = async (name, email, password, course, batch, instructor) => {
    if (!name || !email || !password || !course || !batch || !instructor)
      return sendMessage(false, "Please fill all fields");
    ...
    if (OTPUser) {
      ...
      return sendMessage(false, "Please first verify the email", {
        name,
        userName,
        email,
        password,
    ...
    if (!user && OTPUser) {
      ...
      return sendMessage(
        true,
        "user has bees registered successfully",
        newUser
      );
    }
  };
        ...
        async (err, user, info) => {
          ...
          if (!User.activeStatus) {
            return res.status(403).json({
              status: false,
              message:
                "Plase first click on the link sent to your email to complete you registration",
            });
          }
  ...
  forgotpasswordService = async (email) => {
    ...
    if (email) {
      ...
      if (User) {
        ...
        return sendMessage(true, "Reset password link sent", {
          id: _id,
          email,
          replacements,
        });
  ...
  resetpasswordService = async (newPassword, conformPassword, token, email) => {
    ...
    if (newPassword !== conformPassword) {
      return sendMessage(false, "Please correct conform password", {
        newPassword,
        conformPassword,
        token,
        email,
      ...
      async (error, decode) => {
        ...
        if (decode) {
          ...
          const userid = result.slice(
            0,
            result.length - jwtAccessTokenKey.length
          );
  ...
  changepasswordService = async (
    ...
    if (newPassword !== conformPassword)
      return sendMessage(false, "Please correct the conform password", {
        oldPassword,
        newPassword,
        conformPassword,
        email,
  ...
  refreshTokenService = async (token, email) => {
    const { jwtRefreshTokenKey, jwtAccessTokenKey, jwtAccessTokenExpireTime } =
      environments;

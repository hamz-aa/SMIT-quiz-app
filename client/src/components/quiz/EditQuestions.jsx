...
const EditQuestions = ({ quiz, handleCancel }) => {
    ...
    const fetchQuestions = async () => {
      try {
        const response = await axios.get(
          `${baseUrl}/api/question/get/${quiz._id}`
        );
    ...
    try {
      const questionPromises = questions.map((question) => {
        if (question._id) {
          ...
          return axios.put(
            `${baseUrl}/api/question/update/${question._id}`,
            question
          );
        } else {
          // Create new question
  ...
  return (
    <div className="flex-1 p-4 bg-gray-100">
      ...
      <div className="flex justify-end space-x-4">
        ...
        <PrimaryButton text="Save Quiz" onClick={saveQuestions} />
      </div>
    </div>
  );
};
